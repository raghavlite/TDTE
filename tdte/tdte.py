...
class TDTE(torch.nn.Module):
    def __init__(
        self,
        model_name_or_path: str = None,
        mode: str = 'embedding', # One of ['unified', 'embedding', 'generative']        
        ...
        **kwargs, # Passed to the model, e.g. `attn_implementation`, `torch_dtype` etc.
        ...
        super().__init__()
        if mode == 'embedding':
            peft_config_path = hf_hub_download(repo_id=model_name_or_path, filename="adapter_config.json")
            base_model_name_or_path = json.load(open(peft_config_path))["base_model_name_or_path"]
            base_model = AutoModel.from_pretrained(base_model_name_or_path, trust_remote_code=True, **kwargs)
            ...
            print("Base Model", base_model_name_or_path, "Peft", model_name_or_path, flush=True)
        ...
        if is_inference:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(base_model_name_or_path, padding_side='right')
            # import ipdb; ipdb.set_trace()
    ...
    def save_pretrained(self, temp_path):
        return
    ...
    def encode(
        ...
        **kwargs,
        ...
        for start_index in tqdm(range(0, len(sentences), batch_size), desc="Batches", disable=len(sentences)<256):
            ...
            # print(sentences_batch[0], flush=True)
            # print(sentences_batch[1], flush=True)
            # import ipdb; ipdb.set_trace()
